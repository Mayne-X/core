...
{
    return fmt::format("InitMsgV1, descriptor {}, length {}, worksum {}, grid size {}", descriptor.value(), chainLength.value(), worksum.to_string(), grid.size());
}
...
{
    return fmt::format("InitMsgV1, descriptor {}, length {}, worksum {}, grid size {}", cs.descriptor().value(), cs.headers().length().value(), cs.total_work().to_string(), cs.grid().size());
}
...
{
    return fmt::format("BlockreqMsg {}/[{},{}]", range().descriptor.value(), range().first().value(), range().last().value());
}
...
{
    return fmt::format("InitMsg3, descriptor {}, length {}, worksum {}, grid size {}", descriptor().value(), chain_length().value(), worksum().to_string(), grid().size());
}
...
{
    return fmt::format("InitMsgV3, descriptor {}, length {}, worksum {}, grid size {}", cs.descriptor().value(), cs.headers().length().value(), cs.total_work().to_string(), cs.grid().size());
}
